def calculate_price(is_tuesday: bool, num_pizzas: int, is_delivery: bool, is_ordered_via_app: bool) -> float:
    ...
    # If it's Tuesday apply 50% discount on total price of pizza 
    if is_tuesday:
        total_pizza_price *= 0.5
    
    # Apply 50% discount on total price of pizza if it's Tuesday
    if is_ordered_via_app:
        total_pizza_price *= 0.75
    
    # Calculate final total price by adding pizza cost and delivery cost
